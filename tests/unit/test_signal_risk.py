...
def test_risk_rejects_disabled_account() -> None:
    signal = generate_signal(candles(step=0.8))
    approved, reason, _ = evaluate_risk(signal, minimum_signal_score=65, account_enabled=False, allow_live_trading=False, account_environment="DEMO")
...
def test_risk_rejects_live_when_server_disallows_live() -> None:
    signal = generate_signal(candles(step=0.8))
    approved, reason, _ = evaluate_risk(signal, minimum_signal_score=65, account_enabled=True, allow_live_trading=False, account_environment="LIVE")
    assert approved is False
...
def test_risk_approves_demo_for_simulation() -> None:
    signal = generate_signal(candles(step=0.8))
    approved, reason, details = evaluate_risk(signal, minimum_signal_score=65, account_enabled=True, allow_live_trading=False, account_environment="DEMO")
