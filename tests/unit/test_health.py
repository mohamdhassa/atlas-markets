...
def test_system_info_endpoint():
    r=client.get("/api/system");assert r.status_code==200;p=r.json();assert p["name"]=="ATLAS MARKETS";assert p["phase"]=="29";assert p["account_model"]=="EXTERNAL_PROVIDERS_ONLY";assert p["account_modes"]=="SIMULATION+LIVE_MONEY";assert p["broker_accounts"]=="BYBIT+MT5_FUSION+IBKR";assert p["live_money_execution"]=="GATED";assert p["provider_switching"]=="FRONTEND";assert p["historical_learning"]=="ACTIVE";assert p["ibkr"]=="BRIDGE_READY";assert "METALS" in p["market_scope"]
