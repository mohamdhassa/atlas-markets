...
def test_system_info_endpoint():
    r=client.get("/api/system");assert r.status_code==200;p=r.json();assert p["name"]=="ATLAS MARKETS";assert p["phase"]=="16";assert p["market_data_provider"]=="BYBIT_CRYPTO+TWELVE_DATA_FX";assert p["automation"]=="ACTIVE";assert p["analytics"]=="STRATEGY_INTELLIGENCE";assert p["market_scope"]=="FX_CRYPTO_STOCKS_ETFS"
def test_health_ok(monkeypatch):
    monkeypatch.setattr(health_module,"check_database",lambda:(True,None));monkeypatch.setattr(health_module,"check_redis",lambda:(True,None));r=client.get("/health");assert r.status_code==200;p=r.json();assert p["status"]=="ok"
