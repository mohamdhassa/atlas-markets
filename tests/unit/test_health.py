...
def test_system_info_endpoint():
    r=client.get("/api/system");assert r.status_code==200;p=r.json();assert p["name"]=="ATLAS MARKETS";assert p["phase"]=="36";assert p["account_model"]=="EXTERNAL_PROVIDERS_ONLY";assert p["account_modes"]=="SIMULATION+LIVE_MONEY";assert p["broker_accounts"]=="BYBIT+MT5_FUSION+IBKR";assert p["live_money_execution"]=="GATED";assert p["provider_switching"]=="FRONTEND";assert p["automation"]=="CERTIFIED_ROUTES_ONLY";assert p["certified_automation"]=="MT5_DEMO+IBKR_PAPER";assert p["provider_certification"]=="IBKR_PAPER_CERTIFIED+BYBIT_PROVIDER_BLOCKED";assert p["historical_learning"]=="ACTIVE";assert p["ibkr"]=="PAPER_CERTIFIED_MAX_1_SHARE";assert p["bybit_agent_connect"]=="TESTNET_CONNECTED_EXECUTION_NOT_CERTIFIED";assert "METALS" in p["market_scope"]
