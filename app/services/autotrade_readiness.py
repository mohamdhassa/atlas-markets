    ...
    for cfg in sorted(configs,key=lambda x:(x.market,x.symbol)):
        ...
        try:
            ...
            if profile.provider=='BYBIT':
                candles=await market.get_candles(symbol=cfg.symbol,interval=timeframe,category='linear',limit=200);generated=generate_signal([x.model_dump() for x in candles]);base_url=settings.bybit_demo_base_url if profile.environment=='DEMO' else settings.bybit_testnet_base_url;broker=BybitPrivateClient(decrypt_secret(profile.api_key_encrypted),decrypt_secret(profile.api_secret_encrypted),base_url,settings.market_data_timeout_seconds);wallet=await broker.wallet();positions=await broker.positions();account=(wallet.get('list') or [{}])[0];equity=float(account.get('totalEquity') or account.get('totalWalletBalance') or 0);available=float(account.get('totalAvailableBalance') or account.get('totalWalletBalance') or equity);active=[p for p in positions.get('list',[]) if float(p.get('size') or 0)!=0];existing_positions=len(active);existing_gross=sum(_position_notional(p) for p in active);existing_qty=sum(float(p.get('size') or 0) for p in active if str(p.get('symbol') or '').upper()==cfg.symbol.upper());ticker=await market.get_tickers(category='linear',symbols=(cfg.symbol,));price=float(ticker.tickers[0].last_price) if ticker.tickers else 0
            elif profile.provider=='MT5':
                c=_secret(profile);broker=Mt5BridgeClient(c.get('bridge_url') or 'http://host.docker.internal:8765',c.get('bridge_token'),settings.market_data_timeout_seconds);generated=generate_signal((await broker.candles(cfg.symbol,timeframe,200)).get('list',[]));acct=await broker.account();positions=(await broker.positions()).get('list',[]);equity=float(acct.get('equity') or 0);available=float(acct.get('margin_free') or equity);existing_positions=len(positions);existing_gross=sum(_position_notional(p) for p in positions);existing_qty=sum(float(p.get('volume') or p.get('quantity') or 0) for p in positions if str(p.get('symbol') or '').upper()==cfg.symbol.upper());info=await broker.symbol(cfg.symbol);price=float(info.get('ask') if generated.decision=='BUY' else info.get('bid') or 0);sizing['contract_size']=float(info.get('trade_contract_size') or 100000);sizing['symbol_info']=info
            elif profile.provider=='IBKR':
                c=_secret(profile);broker=IbkrBridgeClient(c.get('bridge_url') or 'http://host.docker.internal:8766',c.get('bridge_token'),settings.market_data_timeout_seconds);generated=generate_signal((await broker.candles(cfg.symbol,timeframe,200,sec_type='STK')).get('list',[]));acct=await broker.account();positions=[p for p in (await broker.positions()).get('list',[]) if float(p.get('quantity') or 0)!=0];equity=float(acct.get('equity') or 0);available=float(acct.get('available') or acct.get('cash') or equity);existing_positions=len(positions);existing_gross=sum(_position_notional(p) for p in positions);existing_qty=sum(float(p.get('quantity') or 0) for p in positions if str(p.get('symbol') or '').upper()==cfg.symbol.upper());price=_ibkr_quote_price(await broker.quote(cfg.symbol,sec_type='STK'),generated.decision)
