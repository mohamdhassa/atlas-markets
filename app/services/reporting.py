...
def snapshot_account(db, profile: BrokerProfile, day: date) -> DailyAccountReport:
    ...
    wallet=db.scalar(select(PaperWallet).where(PaperWallet.profile_id==profile.id));positions=list(db.scalars(select(PaperPosition).where(PaperPosition.profile_id==profile.id)).all())
    ...
    row.realized_pnl=pnl;row.closed_trades=len(exits);row.wins=sum(1 for x in exits if float(x.realized_pnl or 0)>0);row.losses=sum(1 for x in exits if float(x.realized_pnl or 0)<0);row.signals_count=len(signals);row.approved_count=sum(1 for s in signals if s.risk_status=="APPROVED");row.ending_equity=ending;row.starting_equity=ending-pnl;row.generated_at=datetime.now(timezone.utc);return row
