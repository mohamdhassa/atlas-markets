  ...
  try:
   ...
   for account in accounts:
    if account.provider=='BYBIT':
     broker=_bybit_simulation_client(account,settings);wallet=await broker.wallet();broker_positions=await broker.positions();equity,available=_wallet_numbers(wallet);position_rows=[p for p in broker_positions.get('list',[]) if float(p.get('size') or 0)!=0];open_symbols={str(p.get('symbol') or '').upper() for p in position_rows};account.equity_usd=equity;account.available_balance_usd=available;account.open_positions_count=len(position_rows)
     ...
     broker=_mt5_simulation_client(account,settings);acct=await broker.account();health=await broker.health();equity=float(acct.get('equity') or 0);available=float(acct.get('margin_free') or equity);pos=await broker.positions();position_rows=pos.get('list',[]);open_symbols={str(x.get('symbol') or '').upper() for x in position_rows};account.equity_usd=equity;account.available_balance_usd=available;account.open_positions_count=len(position_rows)
