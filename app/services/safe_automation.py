...
def automation_certification_blocker(provider, environment):
    ...
    if (provider, environment) in CERTIFIED_AUTOMATION_ROUTES:
        return None
    if provider == "BYBIT":
        return "BYBIT_TESTNET_OR_DEMO_REQUIRED"
    if provider == "IBKR":
        return "IBKR_PAPER_ONLY_CERTIFIED"
...
def _profile_for_item(db, user_id, item):
    cfg = db.scalar(
        select(SymbolStrategy).where(
            SymbolStrategy.user_id == user_id,
            SymbolStrategy.market == str(item.get("market") or "").upper(),
            SymbolStrategy.symbol == _canonical_symbol(item.get("symbol")),
        )
    )
...
def _broker_ids(broker_result):
    ...
    fill = broker_result.get("fill") or nested.get("fill") or {}
    order_id = (
        broker_result.get("broker_order_id")
        or broker_result.get("order_id")
        or broker_result.get("orderId")
        or broker_result.get("order")
        or broker_result.get("ticket")
        or nested.get("orderId")
        or nested.get("order")
        or nested.get("ticket")
        or fill.get("orderId")
    )
...
def _persist_action(db, scan, user_id, item, result):
    cfg, profile = _profile_for_item(db, user_id, item)
    broker_result = result.get("broker_result") or {}
    qty = result.get("shares")
    if qty is None:
        qty = result.get("volume")
    if qty is None:
        qty = result.get("filled_quantity")
    if qty is None:
        qty = result.get("quantity")
    request = item.get("request") or {}
    if qty is None:
        qty = request.get("shares")
    if qty is None:
        qty = request.get("volume")
    if qty is None:
    ...
    db.add(
        AutomationAction(
            ...
            provider=_short(result.get("provider") or item.get("provider") or "", 32),
            environment=_short(result.get("environment") or (profile.environment if profile else None), 24),
            market=_short(result.get("market") or item.get("market") or "", 24),
            symbol=_short(_canonical_symbol(result.get("symbol") or item.get("symbol")), 32),
            side=_short(result.get("side") or request.get("side"), 8),
            status=_short(result.get("status") or "UNKNOWN", 24),
            reason=_short(result.get("reason"), 128),
            quantity=float(qty) if qty is not None else None,
            sizing_policy=_short(result.get("sizing_policy") or request.get("sizing_policy"), 64),
...
async def _execute_bybit(db, *, user_id, item):
    ...
    cfg = db.scalar(
        select(SymbolStrategy).where(
            ...
            SymbolStrategy.mode == "AUTO_TRADE",
        )
    )
    if cfg is None:
        return {"market": market, "symbol": symbol, "provider": "BYBIT", "status": "SKIP", "reason": "AUTO_TRADE_NOT_ENABLED"}
    profile = db.get(BrokerProfile, cfg.profile_id)
    if profile is None:
        return {"market": market, "symbol": symbol, "provider": "BYBIT", "status": "BLOCK", "reason": "PROFILE_NOT_FOUND"}
    blocker = automation_certification_blocker(profile.provider, profile.environment)
    if blocker:
        return {"market": market, "symbol": symbol, "provider": "BYBIT", "environment": profile.environment, "status": "BLOCK", "reason": blocker}
    proposed = item.get("request") or {}
    side = str(proposed.get("side") or "").upper()
    try:
        quantity = float(proposed.get("quantity") or 0)
    except (TypeError, ValueError):
        quantity = 0.0
    result = await execute_managed_spot_order(
        db,
        profile=profile,
        user_id=user_id,
        symbol=symbol,
...
async def _execute_mt5(db, *, user_id, item):
    ...
    if cfg is None:
        return {"market": market, "symbol": symbol, "provider": "MT5", "status": "SKIP", "reason": "AUTO_TRADE_NOT_ENABLED"}
    profile = db.get(BrokerProfile, cfg.profile_id)
    if profile is None:
        return {"market": market, "symbol": symbol, "provider": "MT5", "status": "BLOCK", "reason": "PROFILE_NOT_FOUND"}
    blocker = automation_certification_blocker(profile.provider, profile.environment)
    if blocker:
        return {"market": market, "symbol": symbol, "provider": profile.provider, "status": "BLOCK", "reason": blocker}
    if not (profile.is_enabled and profile.is_active and profile.credentials_configured and profile.last_connection_status == "CONNECTED"):
        return {"market": market, "symbol": symbol, "provider": profile.provider, "status": "BLOCK", "reason": "ROUTE_NOT_READY"}
    proposed = item.get("request") or {}
    volume = float(proposed.get("volume") or 0)
    stop_loss = proposed.get("stop_loss")
    take_profit = proposed.get("take_profit")
    side = str(proposed.get("side") or "").upper()
    ...
    if "demo" not in server.lower():
        return {"market": market, "symbol": symbol, "provider": "MT5", "status": "BLOCK", "reason": "MT5_DEMO_SERVER_REQUIRED"}
    if terminal and not terminal.get("trade_allowed", False):
        return {"market": market, "symbol": symbol, "provider": "MT5", "status": "BLOCK", "reason": "MT5_ALGO_TRADING_DISABLED"}
    positions = (await broker.positions()).get("list", [])
    if any(_canonical_symbol(x.get("symbol")) == symbol and float(x.get("volume") or 0) != 0 for x in positions):
        return {"market": market, "symbol": symbol, "provider": "MT5", "status": "BLOCK", "reason": "SYMBOL_ALREADY_HAS_POSITION"}
    orders = (await broker.orders()).get("list", [])
    if any(_canonical_symbol(x.get("symbol")) == symbol for x in orders):
        return {"market": market, "symbol": symbol, "provider": "MT5", "status": "BLOCK", "reason": "SYMBOL_ALREADY_HAS_OPEN_ORDER"}
    ...
    if int((check.get("result") or {}).get("retcode", -1)) not in {0, 10009}:
        return {"market": market, "symbol": symbol, "provider": "MT5", "status": "BLOCK", "reason": "BROKER_PREFLIGHT_REJECTED", "broker_check": check}
...
async def _execute_ibkr(db, *, user_id, item):
    ...
    cfg = db.scalar(select(SymbolStrategy).where(SymbolStrategy.user_id == user_id, SymbolStrategy.market == market, SymbolStrategy.symbol == symbol, SymbolStrategy.enabled.is_(True), SymbolStrategy.mode == "AUTO_TRADE"))
    if cfg is None:
        return {"market": market, "symbol": symbol, "provider": "IBKR", "status": "SKIP", "reason": "AUTO_TRADE_NOT_ENABLED"}
    profile = db.get(BrokerProfile, cfg.profile_id)
    if profile is None:
        return {"market": market, "symbol": symbol, "provider": "IBKR", "status": "BLOCK", "reason": "PROFILE_NOT_FOUND"}
    blocker = automation_certification_blocker(profile.provider, profile.environment)
    if blocker:
        return {"market": market, "symbol": symbol, "provider": "IBKR", "status": "BLOCK", "reason": blocker}
    if not (profile.is_enabled and profile.is_active and profile.credentials_configured and profile.last_connection_status == "CONNECTED"):
    ...
    if not (check.get("ok") and check.get("what_if") and check.get("simulation")):
        return {"market": market, "symbol": symbol, "provider": "IBKR", "status": "BLOCK", "reason": "BROKER_WHATIF_REJECTED", "broker_check": check}
    ...
    if verification == "CANCELLED":
        return {**base, "status": "CANCELLED", "reason": "BROKER_ORDER_CANCELLED"}
...
async def run_safe_scan():
    ...
    with SessionLocal() as db:
        ...
        auto_configs = list(db.scalars(select(SymbolStrategy).where(SymbolStrategy.enabled.is_(True), SymbolStrategy.mode == "AUTO_TRADE")).all())
        user_ids = sorted({cfg.user_id for cfg in auto_configs}, key=str)
        if not user_ids:
            state.last_scan_at = now
            state.next_scan_at = now + timedelta(seconds=state.interval_seconds)
            db.commit()
            return {"status": "SKIPPED", "reason": "NO_AUTO_TRADE_SYMBOLS"}

        scan = AutomationScan(status="RUNNING", symbols_count=len(auto_configs), accounts_count=len({cfg.profile_id for cfg in auto_configs}))
        db.add(scan)
        db.commit()
        db.refresh(scan)
        results = []
        try:
            ...
            state.last_scan_at = finished
            state.next_scan_at = finished + timedelta(seconds=state.interval_seconds)
            db.commit()
            return {
                "status": "COMPLETED",
...
async def safe_automation_loop(stop_event):
    while not stop_event.is_set():
        try:
            with SessionLocal() as db:
                state = get_or_create_state(db)
                wait = max(30, state.interval_seconds)
