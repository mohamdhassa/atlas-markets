...
CERTIFIED_AUTOMATION_ROUTES = {('MT5', 'DEMO')}
...
def automation_certification_blocker(provider: str | None, environment: str | None) -> str | None:
    ...
    if provider == 'IBKR':
        return 'IBKR_PAPER_EXECUTION_NOT_CERTIFIED'
...
async def _execute_mt5(db, *, user_id, item: dict) -> dict:
    ...
    check = await broker.order_check({
        'symbol': symbol,
        'side': side,
        'volume': volume,
        'stop_loss': stop_loss,
        'take_profit': take_profit,
        'comment': 'ATLAS AUTO PREFLIGHT',
    })
    retcode = int((check.get('result') or {}).get('retcode', -1))
    ...
    with SessionLocal() as db:
        ...
        try:
                ...
                for item in preflight.get('items', []):
                    ...
                    if provider != 'MT5':
                        results.append({
                            ...
                            'provider': provider,
                            'status': 'BLOCK',
                            'reason': automation_certification_blocker(provider, 'PAPER' if provider == 'IBKR' else None),
                        })
                        continue
            ...
            return {
                ...
                'certified_routes': [{'provider': 'MT5', 'environment': 'DEMO'}],
                'blocked_routes': {'IBKR': 'IBKR_PAPER_EXECUTION_NOT_CERTIFIED', 'BYBIT': 'PROVIDER_EXECUTION_NOT_CERTIFIED'},
            ...
            db.commit()
            return {'status': 'FAILED', 'error': scan.error_message, 'results': results}
