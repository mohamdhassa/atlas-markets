...
async def autotrade_preflight(db, *, user_id) -> dict:
    ...
    configs = {
        (x.market, x.symbol): x
        for x in db.scalars(select(SymbolStrategy).where(SymbolStrategy.user_id == user_id)).all()
    }
    profiles = {
        p.id: p
        for p in db.scalars(select(BrokerProfile).where(BrokerProfile.user_id == user_id)).all()
    }
    items = []
    ...
    for row in readiness['items']:
        ...
        if row.get('readiness') != 'PASS':
            items.append({**base, 'preflight': 'SKIP', 'reason': 'READINESS_BLOCKED'})
            continue
        ...
        try:
            if profile.provider == 'MT5':
                ...
                shares = int(proposed.get('shares') or math.floor(float(proposed.get('quantity') or 0)))
                if shares <= 0:
                ...
                ok = bool(result.get('ok')) and bool(result.get('simulation'))
            else:
            ...
            items.append({
                **base,
                ...
                'reason': None if ok else 'BROKER_PREFLIGHT_REJECTED',
                'request': {
                    'side': proposed.get('side'),
                    'notional': proposed.get('notional'),
                    'shares': proposed.get('shares'),
    ...
    return {
        ...
        'checked_count': sum(x.get('preflight') in {'PASS', 'BLOCK'} and x.get('reason') != 'READINESS_BLOCKED' for x in items),
