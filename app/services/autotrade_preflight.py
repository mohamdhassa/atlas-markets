...
def _mt5_check_ok(result: dict) -> bool:
    ...
    try:
        retcode = int(payload.get("retcode"))
...
def _readiness_reason(row: dict) -> str:
    ...
    return str(reason) if reason else "READINESS_BLOCKED"
...
def _bybit_client(profile, settings) -> BybitPrivateClient:
...
async def autotrade_preflight(db, *, user_id, providers: set[str] | None = None, markets: set[str] | None = None) -> dict:
    ...
    readiness = await autotrade_readiness(db, user_id=user_id)
    settings = get_settings()
    configs = {(x.market, x.symbol): x for x in db.scalars(select(SymbolStrategy).where(SymbolStrategy.user_id == user_id)).all()}
    ...
    for row in readiness["items"]:
        ...
        if provider_filter and row_provider not in provider_filter:
            continue
        if market_filter and row_market not in market_filter:
            continue
        ...
        if row.get("readiness") != "PASS":
            items.append({
                **base,
                "preflight": "SKIP",
                "reason": _readiness_reason(row),
                "readiness_blockers": row.get("blockers") or [],
                "readiness_reason": row.get("reason"),
            })
            continue

        cfg = configs.get((row.get("market"), row.get("symbol")))
        if cfg is None:
            items.append({**base, "preflight": "BLOCK", "reason": "STRATEGY_NOT_FOUND"})
            continue
        profile = profiles.get(cfg.profile_id)
        if profile is None:
            items.append({**base, "preflight": "BLOCK", "reason": "PROFILE_NOT_FOUND"})
            continue
        ...
        try:
            if profile.provider == "BYBIT":
                raw_quantity = proposed.get("quantity")
                try:
                    quantity = float(raw_quantity or 0)
                except (TypeError, ValueError):
                    quantity = 0.0
                if not math.isfinite(quantity) or quantity <= 0:
                    items.append({**base, "preflight": "BLOCK", "reason": "INVALID_BYBIT_SPOT_QUANTITY"})
                    continue
                ...
                pending = [o for o in open_orders if _canonical_symbol(o.get("symbol")) == symbol]
                ok = not pending
                result = {
                    "provider": "BYBIT",
                    ...
                    "open_symbol_orders": len(pending),
                    "managed_inventory": row.get("managed_inventory", row.get("existing_symbol_quantity", 0)),
                }
                ...
                items.append({**base, "preflight": "BLOCK", "reason": "PROVIDER_PREFLIGHT_NOT_SUPPORTED"})
                continue
            ...
            items.append({
                **base,
                ...
                "request": {
                    ...
                    "shares": shares if profile.provider == "IBKR" else proposed.get("shares"),
                    "strategy_requested_shares": proposed.get("strategy_requested_shares"),
                    "sizing_policy": proposed.get("sizing_policy"),
    ...
    return {
        ...
        "scope": {"providers": sorted(provider_filter), "markets": sorted(market_filter)},
        "checked_count": sum(x.get("preflight") in {"PASS", "BLOCK"} for x in items),
        "pass_count": sum(x.get("preflight") == "PASS" for x in items),
        "block_count": sum(x.get("preflight") == "BLOCK" for x in items),
        "skip_count": sum(x.get("preflight") == "SKIP" for x in items),
        "items": items,
    }
