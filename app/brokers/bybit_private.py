...
class BybitPrivateClient:
    ...
    async def place_demo_market_order(self, *, symbol: str, side: str, qty: float, order_link_id: str | None = None) -> dict:
        if self.base_url.rstrip("/") == "https://api.bybit.com":
            raise BybitPrivateError("ATLAS refuses broker-native demo execution on the Bybit LIVE endpoint")
        payload = {
            "category": "linear",
            "symbol": symbol.upper(),
            "side": side,
        ...
        if order_link_id:
            payload["orderLinkId"] = order_link_id[:36]
