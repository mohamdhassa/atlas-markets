...
async function inject(){
 ...
 try{
  const qs=new URLSearchParams({provider:c.provider,markets:c.markets.join(',')});
  ...
  const rows=quotes.symbols||[];const section=document.createElement('section');section.className='panel';section.id='p47LiveSection';
  ...
  section.innerHTML=`<div class="p21-section-head"><div><h3>${esc(c.label)}</h3><div class="muted">Price, movement, latest ATLAS decision and configured execution mode.</div></div><span class="badge good">${rows.length} ${c.badge}</span></div>${rows.length?`<div class="p25-fx-grid">${cards}</div>`:`<div class="empty-state"><p>No live quotes returned for the configured symbols.</p></div>`}${(quotes.errors||[]).length?`<div class="muted" style="margin-top:10px">Quote warnings: ${(quotes.errors||[]).map(x=>`${esc(x.symbol)}: ${esc(x.error)}`).join(' · ')}</div>`:''}`;
  const engine=root.querySelector('section.panel');engine?engine.insertAdjacentElement('afterend',section):root.prepend(section);
 }catch(e){
  if(document.getElementById('pageTitle')?.textContent?.trim()!==name)return;
  const section=document.createElement('section');section.className='panel';section.id='p47LiveSection';section.innerHTML=`<div class="p21-section-head"><div><h3>${esc(c.label)}</h3><div class="muted">Live quote section unavailable.</div></div><span class="badge warn">DATA ERROR</span></div><p class="muted">${esc(e.message)}</p>`;const engine=root.querySelector('section.panel');engine?engine.insertAdjacentElement('afterend',section):root.prepend(section);
 }finally{busy=false}
}
