/* ATLAS MARKETS consolidated frontend core.
   Owns navigation/routing and critical Operations rendering so new work no longer
   depends on phase-by-phase renderPage wrappers or MutationObservers. */
(()=>{
'use strict';
...
function coreBuildNav(){
 const nav=document.getElementById('nav'); if(!nav||!window.state?.user)return;
 nav.innerHTML='';
 const add=(label,pages)=>{const s=document.createElement('div');s.className='nav-separator';s.textContent=label;nav.appendChild(s);pages.forEach(page=>{const b=document.createElement('button');b.className='nav-button';b.dataset.page=page;b.textContent=page;b.addEventListener('click',()=>window.renderPage(page));nav.appendChild(b)})};
...
async function operations(){
 ...
 try{
  ...
  const cards=[];
  for(const a of bybit){
   let spot=null;try{spot=await api(`/accounts/${a.id}/bybit-spot-state`)}catch(e){spot={error:e.message}}
   cards.push(bybitCard(a,spot,actions.filter(x=>String(x.provider).toUpperCase()==='BYBIT')));
  }
  root.innerHTML=`<div class="page-intro"><div><p class="eyebrow">OPERATIONS</p><h3>Execution control center</h3><p class="muted">Live provider readiness, automation state, certification and recent decisions.</p></div>${badge(auto.killed?'KILLED':auto.enabled?'RUNNING':'STOPPED',auto.enabled&&!auto.killed)}</div>
  ...
  ${cards.join('')||'<section class="panel"><h3>Bybit</h3><p class="muted">No Bybit profile configured.</p></section>'}
...
function bybitCard(a,s,actions){
 ...
 const certControls=admin&&sim&&!s.execution_certified?`<div class="atlas-cert"><div><label>Certification symbol<input data-cert-symbol value="BTCUSDT"></label><label>Quantity<input data-cert-qty type="number" min="0.000001" step="0.000001" value="0.0001"></label></div><div class="action-row"><button class="primary-button" data-cert-side="BUY" data-profile="${esc(a.id)}">Certify BUY</button><button class="ghost-button atlas-inline" data-cert-side="SELL" data-profile="${esc(a.id)}">Certify SELL</button><button class="ghost-button atlas-inline" data-reconcile="${esc(a.id)}">Reconcile</button></div><p class="muted">Simulation only. ATLAS verifies provider fill and wallet balance change before recording certification.</p></div>`:'';
...
const legacyRender=window.renderPage;
window.renderPage=async function(page){if(page==='Operations'){if(typeof setActive==='function')setActive(page);return operations()}return legacyRender(page)};
window.buildNav=coreBuildNav;
window.AtlasCore={operations,buildNav:coreBuildNav};
if(window.state?.user)coreBuildNav();
})();
