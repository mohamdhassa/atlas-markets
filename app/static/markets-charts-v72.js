...
async function markets(){
 const active=()=>document.getElementById('pageTitle')?.textContent==='Markets';
 ...
 }catch(e){if(active())content.innerHTML=`<div class="panel empty-state">Market monitor unavailable: ${H(e.message)}</div>`}
}
async function charts(){
 const active=()=>document.getElementById('pageTitle')?.textContent==='Charts';
 ...
 }catch(e){if(active())content.innerHTML=`<div class="panel empty-state">Charts unavailable: ${H(e.message)}</div>`}
}
