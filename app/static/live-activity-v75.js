...
function row(x){const price=x.price??x.execution_price,notional=price!=null&&x.quantity!=null?Number(price)*Number(x.quantity):null;return `<tr data-provider="${E(String(x.provider||'').toUpperCase())}" data-status="${E(String(x.status||'').toUpperCase())}" data-symbol="${E(String(x.symbol||'').toUpperCase())}"><td>${D(x.created_at)}<span class="v75-sub">${E(x.kind)}</span></td><td><strong>${E(brokerName(x))}</strong><span class="v75-sub">${E(x.account_label||'Account not labelled')} · ${E(x.environment||'—')}</span></td><td><strong>${E(x.symbol)}</strong><span class="v75-sub">${E(x.market||'—')}</span></td><td>${E(x.side||'HOLD')}</td><td>${B(x.status,statusKind(x.status))}<span class="v75-sub">${E(x.connection_status||'')}</span></td><td class="v75-reason">${E(x.reason||'—')}<span class="v75-sub">${E(x.sizing_policy||'')}</span></td><td>${N(x.quantity)}</td><td>${M(price)}<span class="v75-sub">${notional==null?'':('Notional '+M(notional))}</span></td><td>${M(x.commission)}</td><td>${x.pnl_available===false?'—':M(x.pnl)}</td><td class="v75-id" title="${E(x.broker_order_id||'')}">${E(x.broker_order_id||'—')}<span class="v75-sub">Pos ${E(x.broker_position_id||'—')}</span></td><td class="v75-id" title="${E(x.scan_id||'')}">${E(x.scan_id||'—')}</td></tr>`}
