...
def actions(limit:int=100,provider:str|None=None,status:str|None=None,symbol:str|None=None,scan_id:uuid.UUID|None=None,current:User=Depends(get_current_user),db:Session=Depends(get_db)):
    q=select(AutomationAction,BrokerProfile).outerjoin(BrokerProfile,BrokerProfile.id==AutomationAction.broker_profile_id)
    ...
    rows=list(db.execute(q.order_by(AutomationAction.created_at.desc()).limit(min(max(limit,1),500))).all())
    return [{"id":r.id,"scan_id":r.scan_id,"user_id":r.user_id,"broker_profile_id":r.broker_profile_id,"provider":r.provider,"broker":profile.provider if profile else r.provider,"account_label":profile.account_label if profile else None,"external_account_ref":profile.external_account_ref if profile else None,"connection_status":profile.last_connection_status if profile else None,"environment":r.environment or (profile.environment if profile else None),"market":r.market,"symbol":r.symbol,"side":r.side,"status":r.status,"reason":r.reason,"quantity":r.quantity,"sizing_policy":r.sizing_policy,"broker_order_id":r.broker_order_id,"broker_position_id":r.broker_position_id,"created_at":r.created_at} for r,profile in rows]
