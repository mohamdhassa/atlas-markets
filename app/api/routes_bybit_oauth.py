...
def start_oauth(payload: OAuthStart, user: User = Depends(get_current_user)):
    """Start Bybit Agent Connect for TESTNET only. No API secret is entered in ATLAS."""
    _cleanup()
    session_id = secrets.token_urlsafe(24)
    state = secrets.token_urlsafe(24)
    verifier = _b64url(secrets.token_bytes(32))
    challenge = _b64url(hashlib.sha256(verifier.encode("ascii")).digest())
    # ATLAS is exposed on Windows as localhost:8000 by the development compose stack.
    redirect_uri = "http://127.0.0.1:8000/api/bybit/oauth/callback"
    params = {
        "client_id": CLIENT_ID,
        "response_type": "code",
        "scope": "ai-account",
    ...
    return {
        ...
        "message": "Open Bybit Testnet and authorize the isolated AI sub-account connection.",
    }
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            response = await client.post(
                ...
                data={"client_id": CLIENT_ID, "code": code, "code_verifier": match["code_verifier"]},
            )
            data = response.json()
        ret = data.get("retCode")
        if ret is not None and ret != 0:
            raise RuntimeError(f"Bybit OAuth {ret}: {data.get('retMsg') or 'token exchange failed'}")
        token = data.get("result") or data
        if not token.get("access_token"):
            raise RuntimeError("Bybit OAuth response did not contain an access token")
        match["access_token"] = token["access_token"]
        match["refresh_token"] = token.get("refresh_token")
        match["status"] = "AUTHORIZED"
        match.pop("code_verifier", None)
        ...
        return HTMLResponse(f"<h2>ATLAS authorization failed</h2><p>{str(exc)[:240]}</p>", status_code=502)
...
async def oauth_status(session_id: str, user: User = Depends(get_current_user)):
    ...
    if s["status"] == "AUTHORIZED":
        ...
        result["accounts"] = [
            {"sub_member_id": str(a.get("sub_member_id")), "nickname": a.get("nickname") or "AI sub-account"}
            for a in accounts
        ]
...
async def select_ai_account(payload: OAuthSelect, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Verify the delivered credentials against the actual TESTNET trading API before persisting them.
    client = BybitPrivateClient(acct["api_key"], acct["api_secret"], TESTNET_TRADING_BASE, get_settings().market_data_timeout_seconds)
    ...
    if payload.activate:
        db.execute(update(BrokerProfile).where(BrokerProfile.user_id == p.user_id, BrokerProfile.provider == "BYBIT", BrokerProfile.id != p.id).values(is_active=False))
        p.is_active = True
    db.commit()
    db.refresh(p)
    s["status"] = "COMPLETE"
    # Do not retain OAuth/API secrets in process memory after ATLAS has encrypted the broker credentials.
