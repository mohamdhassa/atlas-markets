...
# ATLAS_PAPER is intentionally no longer offered for new profiles. Existing rows remain readable
# during the migration period so development history is not destroyed before broker-native demo
# execution and performance ingestion are verified.
...
def _new_profile(db,user,payload):
 if payload.provider=='ATLAS_PAPER':raise HTTPException(410,'ATLAS Paper is deprecated. Connect a broker-native Demo/Testnet/Paper account instead.')
...
def _save_creds(p,api_key=None,api_secret=None,credentials=None):
 if p.provider=='ATLAS_PAPER':raise HTTPException(410,'ATLAS Paper is deprecated')
 if p.provider=='BYBIT':
...
async def _probe(p):
 if p.provider=='ATLAS_PAPER':return 'DEPRECATED','ATLAS Paper is deprecated; move to a broker-native demo account.'
...
def capabilities(user:User=Depends(get_current_user)):
 s=get_settings();return {'providers':{k:sorted(v) for k,v in PROVIDER_ENVIRONMENTS.items()},'trading_providers':sorted(TRADING_PROVIDERS),'credential_fields':{'BYBIT':['api_key','api_secret'],'TWELVE_DATA':['api_key'],'MT5':['login','password','server','bridge_url','bridge_token'],'IBKR':['account_id','host','port','client_id']},'allow_live_trading':bool(s.allow_live_trading),'can_manage_live':_is_admin(user),'broker_native_demo':True,'atlas_paper_deprecated':True}
...
async def sync_account(profile_id:uuid.UUID,user:User=Depends(get_current_user),db:Session=Depends(get_db)):
 p=_authorized_profile(db,user,profile_id)
 try:
  if p.provider=='ATLAS_PAPER':raise HTTPException(410,'ATLAS Paper is deprecated; use broker-native demo performance')
