...
async def test_connection(profile_id:uuid.UUID,user:User=Depends(get_current_user),db:Session=Depends(get_db)):
 ...
 try:
  if p.provider=='ATLAS_PAPER':p.last_connection_status='CONNECTED'
   ...
   data=await _mt5_client(p).account();p.equity_usd=float(data.get('equity') or 0);p.wallet_balance_usd=float(data.get('balance') or 0);p.available_balance_usd=float(data.get('margin_free') or 0);p.last_connection_status='CONNECTED'
  else:
...
async def sync_account(profile_id:uuid.UUID,user:User=Depends(get_current_user),db:Session=Depends(get_db)):
 ...
 try:
  ...
  p.last_connection_status='FAILED';db.commit();raise HTTPException(502,f'sync failed: {str(exc)[:180]}') from exc
