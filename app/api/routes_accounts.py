...
async def sync_account(profile_id: uuid.UUID, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        profile.last_connection_status = "FAILED"
        db.commit()
        raise HTTPException(status_code=502, detail=f"sync failed: {str(exc)[:180]}") from exc
    account = (wallet.get("list") or [{}])[0]
    profile.equity_usd = float(account.get("totalEquity") or 0)
    profile.wallet_balance_usd = float(account.get("totalWalletBalance") or 0)
    profile.available_balance_usd = float(account.get("totalAvailableBalance") or 0)
    profile.open_positions_count = sum(1 for p in positions.get("list", []) if float(p.get("size") or 0) != 0)
    profile.open_orders_count = len(orders.get("list", []))
    profile.last_connection_status = "CONNECTED"
