...
def _base_for_bybit(environment):
    s=get_settings()
    return s.bybit_public_base_url if environment=='LIVE' else s.bybit_demo_base_url if environment=='DEMO' else s.bybit_testnet_base_url
...
def _generic(provider,values):
    c={str(k):str(v).strip() for k,v in (values or {}).items() if str(v).strip()}
    required={'MT5':{'login','password','server'},'IBKR':{'account_id','host','port','client_id'}}.get(provider,set())
...
def _validate_environment(provider,environment):
    if provider=='ATLAS_PAPER': raise HTTPException(410,'ATLAS Paper is deprecated. Connect a broker-native Demo/Testnet/Paper account instead.')
    allowed=PROVIDER_ENVIRONMENTS.get(provider)
    if allowed is None or environment not in allowed: raise HTTPException(400,f'{provider} does not support {environment} environment')

def _duplicate(db,owner_id,provider,environment,label,external_ref=None):
    q=select(BrokerProfile).where(BrokerProfile.user_id==owner_id,BrokerProfile.provider==provider,BrokerProfile.environment==environment)
    rows=list(db.scalars(q).all())
    label_norm=label.strip().lower()
    ref_norm=(external_ref or '').strip().lower()
    for row in rows:
...
async def _validate_raw(payload:BrokerValidateRequest)->BrokerValidationResult:
    _validate_environment(payload.provider,payload.environment)
    warnings=[]; details={}; detected_ref=(payload.external_account_ref or '').strip() or None; detected_name=None
    if payload.provider=='BYBIT':
        ...
        wallet=await client.wallet(); a=(wallet.get('list') or [{}])[0]
        details={'equity':a.get('totalEquity'),'wallet_balance':a.get('totalWalletBalance'),'available_balance':a.get('totalAvailableBalance')}
        return BrokerValidationResult(valid=True,provider='BYBIT',environment=payload.environment,connection_status='CONNECTED',message=f'Bybit {payload.environment} credentials validated successfully.',detected_account_ref=detected_ref,warnings=warnings,details=details)
    if payload.provider=='TWELVE_DATA':
        if not payload.api_key: raise HTTPException(400,'TWELVE DATA requires API key')
        s=get_settings(); quote=await TwelveDataFxMarketData(s.fx_market_data_base_url,payload.api_key.strip(),s.market_data_timeout_seconds).get_quote('EURUSD')
        details={'probe_symbol':'EURUSD','price':getattr(quote,'price',None)}
        return BrokerValidationResult(valid=True,provider='TWELVE_DATA',environment='LIVE',connection_status='CONNECTED',message='Twelve Data API key validated with a live FX request.',warnings=['Twelve Data is market data only; it does not hold trading positions or orders.'],details=details)
    if payload.provider=='MT5':
        c=_generic('MT5',payload.credentials or {})
        bridge=Mt5BridgeClient(c.get('bridge_url') or 'http://host.docker.internal:8765',c.get('bridge_token'),get_settings().market_data_timeout_seconds)
        health=await bridge.health(); account=await bridge.account()
        if not health.get('connected'): raise HTTPException(400,'MT5 bridge is reachable but the terminal is not connected')
        expected_login=str(c['login']).strip(); actual_login=str(account.get('login') or '').strip()
        expected_server=str(c['server']).strip(); actual_server=str(account.get('server') or health.get('server') or '').strip()
        ...
        if terminal.get('trade_allowed') is False: warnings.append('MT5 terminal Algo Trading is currently disabled; connection is valid but automated orders will remain blocked.')
        detected_ref=actual_login; detected_name=account.get('name') or None
        details={'login':actual_login,'server':actual_server,'balance':account.get('balance'),'equity':account.get('equity'),'margin_free':account.get('margin_free'),'trade_allowed':account.get('trade_allowed'),'trade_expert':account.get('trade_expert'),'terminal_trade_allowed':terminal.get('trade_allowed')}
    ...
    if payload.provider=='IBKR':
        _generic('IBKR',payload.credentials or {})
        return BrokerValidationResult(valid=False,provider='IBKR',environment=payload.environment,connection_status='NOT_READY',message='IBKR account validation is not enabled yet. ATLAS will not save or activate this account until the TWS/IB Gateway adapter is implemented.',warnings=['No IBKR profile will be created from this wizard yet.'])
    raise HTTPException(400,'unsupported provider')

def _new_profile(db,user,payload,external_ref=None):
    _validate_environment(payload.provider,payload.environment)
    owner_id=_owner_id(db,user,payload.owner_user_id)
    ref=external_ref if external_ref is not None else ((payload.external_account_ref or '').strip() or None)
    dup=_duplicate(db,owner_id,payload.provider,payload.environment,payload.account_label,ref)
    if dup: raise HTTPException(409,f'duplicate account/profile already exists: {dup.account_label}')
    p=BrokerProfile(user_id=owner_id,provider=payload.provider,account_label=payload.account_label.strip(),environment=payload.environment,external_account_ref=ref,live_execution_enabled=False,is_active=False)
    db.add(p);db.flush();return p

def _save_creds(p,api_key=None,api_secret=None,credentials=None):
    if p.provider=='BYBIT':
        p.api_key_encrypted=encrypt_secret((api_key or '').strip());p.api_secret_encrypted=encrypt_secret((api_secret or '').strip());p.credential_blob_encrypted=None
    elif p.provider=='TWELVE_DATA':
        p.api_key_encrypted=encrypt_secret((api_key or '').strip());p.api_secret_encrypted=None;p.credential_blob_encrypted=None
    else:
...
def capabilities(user:User=Depends(get_current_user)):
    s=get_settings();return {'providers':{k:sorted(v) for k,v in PROVIDER_ENVIRONMENTS.items()},'trading_providers':sorted(TRADING_PROVIDERS),'market_data_only':['TWELVE_DATA'],'credential_fields':{'BYBIT':['api_key','api_secret'],'TWELVE_DATA':['api_key'],'MT5':['login','password','server','bridge_url','bridge_token'],'IBKR':['account_id','host','port','client_id']},'allow_live_trading':bool(s.allow_live_trading),'can_manage_live':_is_admin(user),'broker_native_demo':True,'atlas_paper_deprecated':True,'preflight_validation':True}
...
async def connect_account(payload:BrokerConnectRequest,user:User=Depends(get_current_user),db:Session=Depends(get_db)):
    ...
    if payload.provider=='MT5':
        p.wallet_balance_usd=float(validation.details.get('balance') or 0);p.equity_usd=float(validation.details.get('equity') or 0);p.available_balance_usd=float(validation.details.get('margin_free') or 0)
    if payload.activate:
    ...
    return BrokerConnectResult(profile=p,connected=True,message=(validation.message + (f' {warning_text}' if warning_text else '')),next_action=None)
...
def activate(profile_id:uuid.UUID,user:User=Depends(get_current_user),db:Session=Depends(get_db)):
    ...
    if p.provider=='ATLAS_PAPER':raise HTTPException(410,'ATLAS Paper is deprecated and cannot be activated')
    if p.last_connection_status!='CONNECTED':raise HTTPException(409,'only a successfully connected profile can be made active')
