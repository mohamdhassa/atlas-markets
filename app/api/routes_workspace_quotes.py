...
def _profile(db: Session, user: User, provider: str) -> BrokerProfile | None:
    ...
    if str(user.role) != 'ADMIN' and getattr(user.role, 'value', str(user.role)) != 'ADMIN':
...
def _strategies(db: Session, user: User, provider: str, markets: set[str]) -> list[SymbolStrategy]:
    q = select(SymbolStrategy).join(BrokerProfile, BrokerProfile.id == SymbolStrategy.profile_id).where(
        BrokerProfile.provider == provider,
        SymbolStrategy.enabled.is_(True),
        SymbolStrategy.market.in_(sorted(markets)),
    )
    if str(user.role) != 'ADMIN' and getattr(user.role, 'value', str(user.role)) != 'ADMIN':
        q = q.where(SymbolStrategy.user_id == user.id)
...
async def workspace_quotes(
    ...
    markets: str = Query(min_length=2),
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    provider = provider.upper()
    market_set = {x.strip().upper() for x in markets.split(',') if x.strip()}
    allowed = {'STOCK', 'ETF', 'CRYPTO', 'METAL', 'COMMODITY'}
    market_set &= allowed
    if not market_set:
        raise HTTPException(422, 'no supported markets requested')

    strategies = _strategies(db, user, provider, market_set)
    symbols = list(dict.fromkeys(_canon(x.symbol) for x in strategies))
    rows = []
    errors = []
    ...
    if provider == 'BYBIT':
        try:
            snap = await BybitPublicMarketData(settings.bybit_public_base_url, settings.market_data_timeout_seconds).get_tickers(category='linear', symbols=tuple(symbols))
            by_symbol = {x.symbol: x for x in snap.tickers}
            for cfg in strategies:
                t = by_symbol.get(_canon(cfg.symbol))
                if not t:
                    errors.append({'symbol': cfg.symbol, 'error': 'quote unavailable'})
                    continue
                price = float(t.last_price)
                change_pct = _num(t.change_24h_pct) or 0.0
            ...
            raise HTTPException(502, f'Bybit workspace quotes unavailable: {exc}') from exc

    elif provider == 'IBKR':
        p = _profile(db, user, 'IBKR')
        if not p or not p.credential_blob_encrypted:
            raise HTTPException(503, 'IBKR connected profile unavailable')
        ...
        for cfg in strategies:
            ...
            try:
                ...
                price = _num(q.get('last'), q.get('last_price'), q.get('market_price'), q.get('price'), q.get('close'), q.get('bid'), q.get('ask'))
                bid = _num(q.get('bid'), q.get('bid_price'))
                ask = _num(q.get('ask'), q.get('ask_price'))
                candles = (await broker.candles(symbol, '1d', 2)).get('list', [])
                prev = _num(candles[-2].get('close')) if len(candles) > 1 else price
                change = (price - prev) if price is not None and prev is not None else 0.0
                pct = (change / prev * 100) if prev else 0.0
                if price is None:
                    raise RuntimeError('no usable IBKR quote price')
                ...
                errors.append({'symbol': cfg.symbol, 'error': str(exc)[:180]})

    else:
        p = _profile(db, user, 'MT5')
        if not p or not p.credential_blob_encrypted:
            raise HTTPException(503, 'MT5 connected profile unavailable')
        creds = json.loads(decrypt_secret(p.credential_blob_encrypted))
        ...
        for cfg in strategies:
            symbol = _canon(cfg.symbol)
            try:
                info = await broker.symbol(symbol)
                ...
                pct = (change / prev * 100) if prev else 0.0
                if price is None:
                    raise RuntimeError('no usable MT5 quote price')
    ...
    return {'provider': provider, 'markets': sorted(market_set), 'symbols': rows, 'count': len(rows), 'errors': errors}
