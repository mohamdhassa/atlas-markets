...
async def lifespan(app:FastAPI):
    stop=asyncio.Event();tasks=[asyncio.create_task(safe_automation_loop(stop)),asyncio.create_task(mt5_position_manager_loop(stop)),asyncio.create_task(ibkr_position_manager_loop(stop)),asyncio.create_task(reporting_loop(stop)),asyncio.create_task(historical_loop(stop))];app.state.automation_stop=stop;app.state.background_tasks=tasks
    try:yield
    finally:
        stop.set()
        for task in tasks:
...
_register_missing_router_routes(position_lifecycle_router)
