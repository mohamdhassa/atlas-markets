...
app=FastAPI(title=settings.app_name,version='0.26.0',debug=settings.debug,lifespan=lifespan)
app.include_router(health_router);app.include_router(auth_router);app.include_router(admin_router);app.include_router(markets_router);app.include_router(accounts_router);app.include_router(account_lifecycle_router);app.include_router(analysis_router);app.include_router(signals_router);app.include_router(legacy_account_router);app.include_router(automation_router);app.include_router(broker_native_router);app.include_router(performance_router);app.include_router(news_router);app.include_router(reporting_router);app.include_router(historical_router);app.include_router(symbol_strategies_router);app.mount('/static',StaticFiles(directory=static_dir),name='static')
@app.get('/',include_in_schema=False)
async def root()->FileResponse:return FileResponse(static_dir/'index.html')
@app.get('/api/system',tags=['system'])
async def system_info()->dict[str,str]:return {'name':settings.app_name,'status':'running','phase':'26','account_model':'EXTERNAL_PROVIDERS_ONLY','account_modes':'SIMULATION+LIVE_MONEY','broker_accounts':'BYBIT+MT5_FUSION+IBKR','market_data_providers':'BYBIT+MT5_FUSION+TWELVE_DATA','live_money_execution':'GATED','provider_switching':'FRONTEND','automation':'BROKER_NATIVE_ACCOUNT_AWARE','analytics':'MULTI_MARKET+PER_ACCOUNT+PER_SYMBOL_STRATEGY','historical_learning':'ACTIVE','news_intelligence':'ACTIVE','reporting':'BROKER_NATIVE','market_scope':'FX+CRYPTO+STOCKS+ETFS+METALS+COMMODITIES'}
