...
app=FastAPI(title=settings.app_name,version='0.35.0',debug=settings.debug,lifespan=lifespan)
...
async def system_info()->dict[str,str]:return {'name':settings.app_name,'status':'running','phase':'35','account_model':'EXTERNAL_PROVIDERS_ONLY','account_modes':'SIMULATION+LIVE_MONEY','broker_accounts':'BYBIT+MT5_FUSION+IBKR','market_data_providers':'BYBIT+MT5_FUSION+TWELVE_DATA+IBKR','live_money_execution':'GATED','provider_switching':'FRONTEND','automation':'CERTIFIED_ROUTES_ONLY','certified_automation':'MT5_DEMO+IBKR_PAPER','provider_certification':'IBKR_PAPER_CERTIFIED+BYBIT_PROVIDER_BLOCKED','analytics':'UNIFIED_TRADE_HISTORY+PNL+MULTI_MARKET+PER_ACCOUNT','historical_learning':'ACTIVE','news_intelligence':'ACTIVE','reporting':'BROKER_NATIVE+UNIFIED','market_scope':'FX+CRYPTO+STOCKS+ETFS+METALS+COMMODITIES','ibkr':'PAPER_CERTIFIED_MAX_1_SHARE','bybit_agent_connect':'TESTNET_CONNECTED_EXECUTION_NOT_CERTIFIED'}
